...
pub fn handler(ctx: Context<NftMintByCoin>) -> Result<()> {
    //todo:
    //1. 扣钱支持sol和usdc
    //2. id自增
    //3，考虑master_edition
    ...
    ctx.accounts.collection_state.amount = current_id;
    if ctx.accounts.user_state.pending_airdrop_count > 0 {
        ...
        msg!("pending_airdrop_count is zero, start pay coin");
        let cpi_accounts = Transfer {
            from: ctx.accounts.user_settle_token_ata.to_account_info(),
            to: ctx.accounts.treasurer_settle_token_ata.to_account_info(),
            authority: ctx.accounts.payer.to_account_info(),
        };
        let cpi_program = ctx.accounts.token_program.to_account_info();
        let cpi_ctx = CpiContext::new(cpi_program, cpi_accounts);
        let pay_amount = ctx.accounts.collection_state.settle_token_price;
        token::transfer(cpi_ctx, pay_amount)?;
        ctx.accounts.collection_state.settle_token_income += pay_amount;
...
pub struct NftMintByCoin<'info> {
    ...
    #[account(
        ...
        seeds = [MINT_SEED.as_bytes(),collection_mint.key().as_ref(),&(collection_state.amount + 1).to_le_bytes()[..]], 
        bump,
    )]
    ...
    #[account(mut,associated_token::mint = collection_state.settle_token,associated_token::authority = payer)]
    pub user_settle_token_ata: Box<InterfaceAccount<'info, TokenAccountInterface>>,

    #[account(mut,associated_token::mint = collection_state.settle_token,associated_token::authority = admin_state.treasurer)]
    pub treasurer_settle_token_ata: Box<InterfaceAccount<'info, TokenAccountInterface>>,
}
