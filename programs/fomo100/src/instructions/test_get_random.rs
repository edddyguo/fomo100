use super::test_random::PLAYER_STATE_ACCOUNT_SEED;
...
pub fn handler(ctx: Context<TestGetRandom>, force: [u8; 32]) -> Result<()> {
    let player_state = &mut ctx.accounts.player_state;
    let random_account = &ctx.accounts.random;
    let force = player_state.force;
    msg!("force value {:?}", force);
    let random_value = player_state.get_random_value(random_account);
