...
pub fn handler(ctx: Context<Stake>, amount: u64) -> Result<()> {
    ...
    if pool_store.is_empty(){
       //reward 直接继承
       pool_store.push(Round { round_index: current_round_index,reward_index:current_reward_index, stake_amount:amount.view() })?;
    ...
    //如果history_rounds的最后一个值不等于current_round_index则说明，当前为这个轮次的第一个stake
        ...
        pool_store.push(Round { round_index: current_round_index,reward_index:current_reward_index, stake_amount })?;
    }else {
        unreachable!("")
    }
